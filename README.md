...
Глобальная балансировка нагрузки распределяет запросы между дата‑центрами, снижает задержки, повышает отказоустойчивость и позволяет масштабировать компоненты независимо. Для Gemini с 750 млн MAU и пиковым внешним RPS ~11 735 это критически важно.
...
<img width="1308" height="877" alt="image" src="https://github.com/user-attachments/assets/bc17df99-36a1-425a-842a-f682efd4037b" />


Локации выбраны с учётом географии пользователей (США ~40 %, Азия ~30 %, Европа ~20 %, остальные ~10 %), требований законодательства (GDPR) и наличия инфраструктуры Google Cloud.

| ID | Локация | Обслуживаемый регион | Доля пользователей |
|----|---------|----------------------|-------------------|
| DC1, DC3 | Майами (США) | Северная и Южная Америка | 40 % (совместно) |
| DC2, DC4 | Амстердам (Нидерланды) | Европа, Африка, Ближний Восток | 20 % (совместно) |
| DC5 | Сингапур (Сингапур) | Азия и Океания | 30 % |

Обоснование: 
- Майами – близость к США и Бразилии, развитая сеть.  
- Амстердам – крупнейшая точка обмена трафиком в Европе.  
- Сингапур – ключевой узел Азиатско-Тихоокеанского региона, минимальные задержки для Индии и Юго-Восточной Азии.
...
|-------------|--------------------|------------------------|------------------------------|
| Северная Америка (DC1, DC3) | 40 % | 4 694 | DC1: 2 347, DC3: 2 347 (50/50) |
| Европа (DC2, DC4) | 20 % | 2 347 | DC2: 1 173, DC4: 1 174 (50/50) |
| Азия (DC5) | 30 % | 3 520 | DC5: 3 520 |
| Остальные регионы (проксируются) | 10 % | 1 174 | Проксируются через ближайший ДЦ |
| Итого | 100 % | 11 735 | |

Трафик из регионов без собственного ДЦ (Африка, Австралия) направляется в ближайший ДЦ и при необходимости проксируется к «родному» ДЦ пользователя.
...
|-------|----------------------------------|-------------------|
| `api.gemini.google.com` | Основные API‑запросы | IP ближайшего ДЦ (Азия → DC5; Европа/Африка → DC2/DC4; Америки → DC1/DC3) |
| `history.gemini.google.com` | Получение списка диалогов, загрузка диалога | IP ближайшего ДЦ |
| `media.gemini.google.com` | Загрузка и скачивание исходных медиа | IP ближайшего ДЦ |
| `static.gemini.google.com` | Статические ресурсы | IP ближайшего ДЦ (Anycast через Cloud CDN) |
...
Особенности:  
- Загрузка медиа в File API не требует проксирования – файл сохраняется локально.  
- Для Gemini Live (WebSocket) применяется session affinity.  
- При длительном нахождении в чужом регионе (более 7 дней) запускается фоновая миграция данных.

## 3.5 Механизмы регулировки трафика между ДЦ

| Механизм | Описание |
|----------|----------|
| Weighted Round‑Robin (DNS) | В Geo‑DNS задаются веса, пропорциональные ёмкости ДЦ. При плановых работах вес снижается, трафик перераспределяется. |
...
| Автоматическое масштабирование | Горизонтальное масштабирование внутри ДЦ (GKE, Cloud Run) под текущую нагрузку. |

Данное распределение по регионам позволяет:
- Разнести пиковый RPS 11 735 на 5 кластеров с нагрузкой от 1 173 до 3 520 RPS каждый.
- Сократить межрегиональный трафик: около 80–90 % запросов обрабатывается локально.
- Распределить временное хранение: Америка ~1,6 ПБ (на два ДЦ), Европа ~0,8 ПБ, Азия ~1,2 ПБ.

---

# 4. Локальная балансировка нагрузки

Внутри каждого дата‑центра нагрузка распределяется между серверами приложений и сервисами с использованием двухуровневой схемы балансировки, обеспечивающей высокую доступность и масштабируемость.
...
Расчёт выполнен для наиболее загруженного дата‑центра – DC1/DC3 (Америка), так как на него приходится максимальный объём трафика (40 % от общей нагрузки). Исходные данные для расчёта получены из раздела 2.2:

- Пиковый входящий трафик (Америка): 99,73 × 0,4 ≈ 39,9 Гбит/с  
- Пиковый исходящий трафик (Америка): 1 130,4 × 0,4 ≈ 452,2 Гбит/с  
- Суммарный трафик через балансировщики L7: 39,9 + 452,2 ≈ 492,1 Гбит/с  
- Пиковый RPS (Америка): 11 735 × 0,4 ≈ 4 694 запросов/с  

### 4.2.1 Расчёт узлов L4

Целевая конфигурация узлов L4 – серверы с сетевыми интерфейсами 100 Гбит/с. Ограничивающим фактором выступает пропускная способность канала (входящий трафик).

Активных узлов = Входящий трафик / 100 Гбит/с = 39,9 / 100 ≈ 0,4 → округляем до 1
...
Узлы L7 конфигурируются с 16 физическими ядрами CPU и сетевыми интерфейсами 100 Гбит/с. При расчёте учитываются два ограничителя: пропускная способность и производительность SSL‑терминации.

По пропускной способности:

Активных узлов = Суммарный трафик / 100 Гбит/с = 492,1 / 100 ≈ 4,9 → 5 узлов

По производительности SSL‑терминации:

Интенсивность новых TLS‑соединений в секунду (CPS) в худшем случае может достигать величины RPS (при отсутствии keep‑alive). Согласно тестам NGINX, сервер с 16 ядрами обрабатывает 6 676 SSL‑соединений в секунду.

Активных узлов = RPS (Америка) / 6 676 = 4 694 / 6 676 ≈ 0,7 → 1 узел

Более строгим ограничением является пропускная способность (5 узлов). Выбираем худший случай:
...
| L7 | 6 | CPU 16 ядер, NIC 100 Гбит/с, NGINX | N + 1 |
