...
Фаза 1 (Prepare): Воркер message‑writer, который будет координировать две эти операции, отправляет Prepare‑запросы одновременно в ScyllaDB (записать сообщение с флагом prepared) и в PostgreSQL (обновить updated_at с флагом prepared). Оба хранилища резервируют изменения, но не фиксируют их.
...
Отдельный процесс Outbox Relay (лёгкий поток в том же сервисе) непрерывно читает неотправленные строки из outbox (SELECT … WHERE sent = false) и публикует их в Kafka.

После успешной публикации строка помечается как отправленная.

Таким образом, даже если Kafka временно недоступен, событие гарантированно сохранено в PostgreSQL и будет отправлено позже. На схеме это отражено связью PG → topic outbox‑events
...
flowchart TD
    ...
    subgraph Kafka["Apache Kafka (Brokers)"]
        TopicUser["topic: user‑messages"]
        TopicMedia["topic: media‑events"]
        TopicOutbox["topic: outbox‑events"]
    end
    ...
    MediaService -- "publish media event" --> TopicMedia

    TopicUser --> MessageWriter
    TopicUser --> EmbeddingWorker
    TopicMedia --> ASRWorker
    TopicMedia --> MediaTranscoder
...
Расчёты выполнены раздельно для двух профилей дата-центров:

- Профиль A (высокая нагрузка): DC1 (15 %), DC5 (15 %), DC4 (15 %), DC3 (12,5 %), DC7 (12,5 %). Суммарно 70 % мирового RPS.
- Профиль B (умеренная нагрузка): DC2 (10 %), DC6 (10 %). Суммарно 20 % мирового RPS.

Оставшиеся 10 % мирового трафика проксируются на ближайший ДЦ и равномерно добавляются к нагрузке профилей. Для профиля A пиковая доля одного ДЦ принимается равной 17 % (15 % + 2 % proxy), для профиля B — 12 % (10 % + 2 %). Исходные мировые пиковые значения (раздел 2.2): RPS = 11 735, входящий трафик = 99,73 Гбит/с, исходящий = 1 130,4 Гбит/с.

## 11.1 Ресурсные требования сервисов
...
Сервисы и потребность в ресурсах (профиль A, 17 % пиковой нагрузки, с запасом +20 %)

| Сервис | Тип нагрузки | Пиковая нагрузка | CPU (ядер) | RAM |
|--------|--------------|------------------|------------|-----|
| LVS (L4) | Лёгкая (сеть) | 16,95 Гбит/с вх. | 8 (1 узел) | 8 ГБ |
| Nginx (L7) | Средняя (SSL+прокси) | 1 995 CPS / 209,1 Гбит/с | 60 (3×20) | 60 ГБ |
| api‑gateway | Лёгкая (JSON API) + SSE | 1 344 RPS + ~6 800 одновременных SSE | 20 (5×4) | 40 ГБ |
| history‑service | Средняя (запросы к БД) | 650 RPS | 4 (1×4) | 4 ГБ |
| media‑service | Тяжёлая (сеть+IO) | 384 RPS + 209,1 Гбит/с | 72 (9×8) | 72 ГБ |
| auth‑service | Лёгкая (JSON API) | 300 RPS | 4 (1×4) | 4 ГБ |
| message‑writer | Средняя (2PC‑логика) | ~2 690 сообщ./с | 12 (3×4) | 12 ГБ |
| embedding‑worker | Тяжёлая (Python ML) | ~2 690 эмбеддингов/с | 40 (5×8) | 20 ГБ |
| asr‑worker | Лёгкая (интеграция) | 97 RPS | 4 (2×2) | 4 ГБ |
| media‑transcoder | Тяжёлая (Python) | 385 событий/с | 40 (5×8) | 40 ГБ |
| PostgreSQL | Средняя (OLTP) | 2 550 write + 29 750 read QPS | 20 | 80 ГБ |
| ScyllaDB | Тяжёлая (NoSQL) | 53 200 write + 177 300 read ops/с | 84 (3×28) | 224 ГБ |
| Redis Cluster | Лёгкая (in‑memory) | ~127 000 ops/с | 12 | 48 ГБ |
| Kafka | Средняя (брокер) | 21 300 сообщ./с | 24 (1 брокер) | 48 ГБ |
| Ceph RGW | Тяжёлая (объекты) | 16,95 Гбит/с вх. / 192,17 Гбит/с исх. | 40 (2×20) | 80 ГБ |
| Ceph OSD | Средняя (хранение) | ~97 ТБ стартового хранения | 8 | 16 ГБ |

Сервисы и потребность в ресурсах (профиль B, 12 % пиковой нагрузки, с запасом +20 %)

| Сервис | Тип нагрузки | Пиковая нагрузка | CPU (ядер) | RAM |
|--------|--------------|------------------|------------|-----|
| LVS (L4) | Лёгкая (сеть) | 11,97 Гбит/с вх. | 8 (1 узел) | 8 ГБ |
| Nginx (L7) | Средняя (SSL+прокси) | 1 408 CPS / 147,6 Гбит/с | 48 (3×16) | 48 ГБ |
| api‑gateway | Лёгкая (JSON API) + SSE | 950 RPS + ~4 800 одновременных SSE | 16 (4×4) | 32 ГБ |
| history‑service | Средняя (запросы к БД) | 460 RPS | 4 (1×4) | 4 ГБ |
| media‑service | Тяжёлая (сеть+IO) | 270 RPS + 147,6 Гбит/с | 56 (7×8) | 56 ГБ |
| auth‑service | Лёгкая (JSON API) | 212 RPS | 4 (1×4) | 4 ГБ |
| message‑writer | Средняя (2PC‑логика) | ~1 900 сообщ./с | 8 (2×4) | 8 ГБ |
| embedding‑worker | Тяжёлая (Python ML) | ~1 900 эмбеддингов/с | 32 (4×8) | 16 ГБ |
| asr‑worker | Лёгкая (интеграция) | 68 RPS | 4 (2×2) | 4 ГБ |
| media‑transcoder | Тяжёлая (Python) | 270 событий/с | 32 (4×8) | 32 ГБ |
| PostgreSQL | Средняя (OLTP) | 1 800 write + 21 000 read QPS | 16 | 64 ГБ |
| ScyllaDB | Тяжёлая (NoSQL) | 37 500 write + 125 000 read ops/с | 72 (3×24) | 192 ГБ |
| Redis Cluster | Лёгкая (in‑memory) | ~90 000 ops/с | 8 | 32 ГБ |
| Kafka | Средняя (брокер) | 15 000 сообщ./с | 16 (1 брокер) | 32 ГБ |
| Ceph RGW | Тяжёлая (объекты) | 11,97 Гбит/с вх. / 135,65 Гбит/с исх. | 32 (2×16) | 64 ГБ |
| Ceph OSD | Средняя (хранение) | ~68 ТБ стартового хранения | 8 | 16 ГБ |

Пояснения к отдельным сервисам:

- api‑gateway принимает все пользовательские промпты и стримит ответы через SSE. RPS-часть лёгкая, а длинные соединения требуют памяти и дескрипторов.
- history‑service обслуживает домен history.*: выдача списка чатов и загрузка сообщений.
- message‑writer асинхронно читает из Kafka завершённые диалоги, выполняет двухфазный коммит (2PC) для одновременной записи сообщения в ScyllaDB и обновления `updated_at` чата в PostgreSQL.
- embedding‑worker генерирует векторы для семантического поиска, используя Python и pgvector.
- asr‑worker распознаёт речь, вызывая Google Speech-to-Text API.
- media‑transcoder создаёт сжатые превью (WebP/VP9) для загружаемых медиафайлов.
- Nginx выполняет SSL‑терминацию и маршрутизацию, основное ограничение — пропускная способность.
- Потребности stateful-систем получены умножением мировых пиковых QPS (раздел 5) на долю ДЦ с запасом 30–50 %.

## 11.2 Расчёт Kubernetes‑узлов

Профиль A (17 %). Суммарный CPU request всех stateless‑подов:
...
С учётом системных подов и накладных расходов Kubernetes (+20%): ~235 ядер. Один K8s-узел: 2×AMD EPYC 7543, 64 ядра, 512 ГБ RAM. Необходимое количество: ⌈235 / 64⌉ = 4, плюс один резервный узел — 5 узлов на каждый ДЦ профиля A.

Профиль B (12 %). Суммарный CPU request статeless‑подов:
...
С учётом overhead (+20%): ~187 ядер. ⌈187 / 64⌉ = 3, плюс резервный — 4 узла на каждый ДЦ профиля B.

## 11.3 Конфигурации физических серверов (весь проект, амортизация 5 лет)

| Название | Конфигурация | Ядра | Кол‑во | Покупка ($) | Аренда/мес ($) |
|----------|--------------|------|--------|-------------|----------------|
| kubenode (A) | 2×AMD EPYC 9454P / 16×32GB DDR5 / 2×Samsung 4TB NVMe / 2×25GbE | 48 | 20 | 24 500 | 489 (Hetzner AX162‑S) |
| kubenode (B) | 2×Intel Xeon Silver 4314 / 16×32GB DDR5 / 2×Samsung 4TB NVMe / 2×25GbE | 32 | 8 | 13 200 | 260 (Hetzner AX102) |
| lvs-l4 | 1×Intel Xeon E‑2334 / 4×8GB / 1×NVMe 512GB / 2×100GbE | 8 | 14 | 4 800 | 73 (аналог Hetzner EX101) |
| nginx-l7 (A) | 2×Intel Xeon Silver 4314 / 4×16GB / 2×NVMe 512GB / 2×100GbE | 16 | 15 | 6 200 | 106 (Hetzner EX101) |
| nginx-l7 (B) | 2×Intel Xeon Silver 4314 / 4×16GB / 2×NVMe 512GB / 2×100GbE | 16 | 6 | 6 200 | 106 (Hetzner EX101) |
| db-postgres | 2×AMD EPYC 7282 / 8×32GB / 4×NVMe 4TB / 2×25GbE | 32 | 32 | 16 500 | 275 (Hetzner AX102) |
| db-scylla | 2×AMD EPYC 7282 / 8×32GB / 8×NVMe 8TB / 2×25GbE | 32 | 21 | 24 000 | 400 (Hetzner AX102) |
| db-redis | 2×Intel Xeon Silver 4210R / 16×32GB / 2×NVMe 512GB / 2×25GbE | 20 | 42 | 13 500 | 225 (Hetzner EX101) |
| kafka | 2×Intel Xeon Silver 4210 / 4×32GB / 4×NVMe 4TB / 2×25GbE | 16 | 21 | 9 100 | 152 (Hetzner EX101) |
| ceph-mon | 1×Intel Xeon E‑2334 / 2×16GB / 2×NVMe 512GB / 2×10GbE | 8 | 9 | 4 800 | 73 (Hetzner EX101) |
| ceph-rgw | 2×AMD EPYC 7282 / 4×32GB / 2×NVMe 2TB / 2×100GbE | 32 | 9 | 11 200 | 188 (Hetzner EX101) |
| ceph-osd | 1×Intel Xeon E‑2334 / 4×16GB / 12×HDD 18TB / 2×25GbE | 8 | 55* | 20 500 | 340 (Hetzner SX133) |
...
Для покупки все цены основаны на розничных предложениях от проверенных реселлеров. Например, процессоры AMD EPYC и Intel Xeon доступны на Newegg, CDW или Scan.co.uk. Серверные платформы и barebone-системы (Supermicro) — на Thinkmate или у локальных сборщиков. Накопители (NVMe SSD, HDD) также можно приобретать централизованно у тех же реселлеров.

Для аренды все решения закрываются одним провайдером — Hetzner. Вычислительные узлы (kubenode) берутся из линейки AX, простые серверы (lvs, nginx, redis) — из линейки EX, а холодное файловое хранилище (ceph-osd) оптимально размещать на SX-серверах

## 11.4 Распределение серверов по дата-центрам

| Тип | Профиль A (5 ДЦ) | Профиль B (2 ДЦ) | Всего |
|-----|------------------|------------------|-------|
| kubenode | 5×5 = 25 | 4×2 = 8 | 33 (с учётом резерва) |
| lvs-l4 | 2×5 = 10 | 2×2 = 4 | 14 |
| nginx-l7 | 3×5 = 15 | 3×2 = 6 | 21 |
| db-postgres | 6×5 = 30 (включая шарды) | 4×2 = 8 | 38 |
| db-scylla | 3×5 = 15 | 3×2 = 6 | 21 |
| db-redis | 6×5 = 30 | 6×2 = 12 | 42 |
| kafka | 3×5 = 15 | 3×2 = 6 | 21 |
...
## 11.5 Аллокация ресурсов в Kubernetes

Профиль A (на один ДЦ):

| Сервис | CPU requests | CPU limits | RAM requests | RAM limits | Реплики |
|--------|--------------|------------|---------------|------------|---------|
| api‑gateway | 4 | 4 | 8 GB | 16 GB | 5 |
| history‑service | 4 | 4 | 4 GB | 8 GB | 1 |
| media‑service | 8 | 8 | 8 GB | 16 GB | 9 |
| auth‑service | 4 | 4 | 2 GB | 4 GB | 1 |
| message‑writer | 4 | 4 | 4 GB | 8 GB | 3 |
| embedding‑worker | 8 | 8 | 4 GB | 8 GB | 5 |
| asr‑worker | 2 | 2 | 2 GB | 4 GB | 2 |
| media‑transcoder | 8 | 8 | 8 GB | 16 GB | 5 |

Профиль B (на один ДЦ):

| Сервис | CPU requests | CPU limits | RAM requests | RAM limits | Реплики |
|--------|--------------|------------|---------------|------------|---------|
| api‑gateway | 4 | 4 | 8 GB | 16 GB | 4 |
| history‑service | 4 | 4 | 4 GB | 8 GB | 1 |
| media‑service | 8 | 8 | 8 GB | 16 GB | 7 |
| auth‑service | 4 | 4 | 2 GB | 4 GB | 1 |
...
CPU limits равны CPU requests — throttling исключён. RAM limits удвоены относительно requests как страховка от пиковых всплесков. Для всех Go-сервисов установлена переменная окружения GOMEMLIMIT=90 % от RAM limits, чтобы сборщик мусора агрессивнее освобождал память и предотвращал OOM.

## 11.6 Сравнение стоимости покупки и аренды 

| Компонент | Решение | Обоснование |
|-----------|---------|-------------|
| GPU‑кластер (H100) | Покупать | Покупка ($320 000) при амортизации 5 лет даёт $5 333/мес, аренда у hyperscaler — около $12 000/мес. Экономия $6 667/мес с одного сервера, для кластера из 10 узлов — около $800 000/год. |
| Kubernetes‑узлы (kubenode A) | Покупать | Покупка $24 500 → амортизация $408/мес против аренды Hetzner AX162‑S $489/мес. Выгода $81/мес на узел; при 20 узлах — $19 440/год. |
| Kubernetes‑узлы (kubenode B) | Покупать | Покупка $13 200 → амортизация $220/мес против аренды Hetzner AX102 $260/мес. Выгода $40/мес на узел. |
| LVS, Nginx | Покупать | Низкая стоимость покупки (от $4 800) окупается за 2–3 года по сравнению с арендой EX101 за $73–106/мес. Полный контроль над конфигурацией. |
| PostgreSQL, ScyllaDB, Redis, Kafka | Покупать | Покупка и аренда близки по стоимости (часто паритет, например db‑postgres $275/мес при покупке против $275/мес аренды). При длительной эксплуатации покупка исключает рост арендной платы и даёт контроль над дисками. |
| Ceph OSD (холодное хранение) | Арендовать | Покупка $20 500 → амортизация $342/мес, аренда Hetzner SX133 — $280–340/мес. С учётом износа HDD и необходимости замены каждые 3–5 лет аренда снижает операционные риски и часто дешевле. |
| Ceph RGW, MON | Покупать | Стабильная нагрузка, редко меняющееся железо. Покупка $11 200 (RGW) и $4 800 (MON) окупается по сравнению с арендой EX101 за $188 и $73/мес соответственно. |

Таким образом, основная вычислительная инфраструктура и GPU выгоднее приобрести в собственность, а массивное файловое хранилище на HDD — арендовать, что формирует оптимальную по совокупной стоимости владения гибридную модель.
