...
| :--- | :--- | :--- | :--- |
| users | `email`, `phone` | UNIQUE B-Tree | Быстрый поиск пользователя при аутентификации по email или номеру телефона. |
| chats | `(owner_id, updated_at DESC)` | Composite B-Tree | Используется для получения списка диалогов пользователя с сортировкой по дате последнего обновления. |
| media_metadata | `(chat_id, message_id)` | B-Tree | Быстрый поиск превью, прикрепленных к конкретному сообщению в чате. 
| messages | `(chat_id)`, `message_id` | Partition Key + Clustering Key DESC | ScyllaDB не использует классические индексы; структура первичного ключа определяет физическое размещение данных. chat_id как ключ партиционирования гарантирует локальность всех сообщений одного чата на одном узле, а message_id с сортировкой по убыванию позволяет мгновенно получить последние N сообщений (LIMIT 50). |
| embeddings | `embedding` | IVF_FLAT | Индекс для приближенного поиска ближайших соседей (ANN) по косинусному расстоянию. |

## 6.4. Шардирование и резервирование

| Компонент | Стратегия шардирования / партиционирования | Схема резервирования |
| :--- | :--- | :--- |
| PostgreSQL | Шардирование по `user_id` с использованием Citus или ручного прокси. Данные пользователя и его чаты локализованы на одном шарде. | Master-Replica (1 Master + 2 Replica). Запись в мастер, чтение с реплик. Если главный упал — копия становится главной  |
| ScyllaDB | Partition Key = `chat_id`. Каждый чат целиком лежит на одном наборе узлов (vnodes). | Данные хранятся на трех узлах в разных стойках. Данные надежно защищены при любом падении узла |
| Redis | Redis Cluster. Ключи распределены по слотам равномерно (например, `session:{token}`). | Master-Slave. Каждый мастер имеет минимум одного реплицирующего слейва. При падении мастера слейв автоматически повышается оркестратором кластера. |
| Ceph RGW | Шардирование на уровне бакетов: `bucket = user_id % N`. | Данные реплицируются между регионами. |
...
4. Алгоритм:
   ...
   2. Pre-Processing (асинхронная обработка):  
   ...
   3. Model Inference (взаимодействие с LLM):  
   ...
   5. Write Path (сохранение в постоянную историю): 
...
1. Блок системы: API Gateway / SSE Handler (Backend Core Services).
...
1. Инициация стриминга клиентом
...
2. Проксирование запроса к модели
...
3. Установка SSE-соединения с клиентом
...
4. Потоковая отправка токенов (SSE события)
При получении очередного токена от модели (через gRPC stream) сервер:

- Упаковывает токен в JSON-фрагмент: {"token": "частичный_текст", "finish_reason": null}.

- Отправляет клиенту строку формата SSE:

text
data: {"token": "Сегодня", "finish_reason": null}
(каждое событие завершается двойным переводом строки \n\n).

- Если модель возвращает несколько токенов за один вызов gRPC, каждый токен отправляется отдельным SSE-событием.

5. Обновление интерфейса клиентом
Клиентский JavaScript (EventSource) при получении каждого события дополняет отображаемый текст новым токеном. Пользователь видит, как ответ «печатается» в реальном времени.

6. Завершение генерации
...
7. Обработка ошибок и обрывов соединения

- При ошибке на стороне модели (например, превышение лимита токенов) сервер отправляет SSE-событие с полем error и закрывает стрим.

- Если соединение с клиентом обрывается до завершения, сервер прекращает gRPC-стрим к модели и освобождает ресурсы. Клиент при обнаружении разрыва может отобразить сообщение о сетевой ошибке и предложить повторную отправку (новый запрос).

- Для контроля состояния соединения используются keep-alive комментарии SSE: каждые 15 секунд сервер отправляет : ping\n\n, чтобы предотвратить закрытие простоя прокси-серверами.

5. Обоснование
Server-Sent Events (SSE) оптимальны для однонаправленного потокового вещания текста от сервера к клиенту. В отличие от WebSocket, SSE проще в реализации, нативны для HTTP/2, не требуют ручного управления фреймами и легко проксируются стандартными балансировщиками. Потоковая выдача токенов сокращает Time To First Byte (TTFB) и позволяет пользователю начинать чтение ответа до его полной генерации, что критически важно для длинных ответов и удержания вовлечённости. gRPC stream обеспечивает эффективное внутреннее взаимодействие с моделью с минимальными накладными расходами на сериализацию.
